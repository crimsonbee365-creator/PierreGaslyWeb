...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if (empty($error)) {
            $sql = "INSERT INTO products (category_id, brand_id, product_name, size_kg, price, stock_quantity, minimum_stock, description, product_image, availability, created_by) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            if ($db->query($sql, [$category_id, $brand_id, $product_name, $size_kg, $price, $stock_quantity, $minimum_stock, $description, $product_image, $availability, $_SESSION['user_id']])) {
...
if (isset($_GET['archive'])) {
    ...
    if ($product) {
        if ($db->query("UPDATE products SET is_active = 0 WHERE product_id = ?", [$product_id])) {
...
if (isset($_GET['restore'])) {
    ...
    if ($product) {
        if ($db->query("UPDATE products SET is_active = 1 WHERE product_id = ?", [$product_id])) {
