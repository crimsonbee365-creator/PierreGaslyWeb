...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_rider'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if (!preg_match('/^09\d{9}$/', $phone))
            $validation_errors[] = 'Phone must be a valid Philippine mobile number (09XXXXXXXXX)';
...
$sql = "SELECT u.*, 
        (SELECT COUNT(*) FROM orders WHERE customer_id = u.user_id AND order_status = 'delivered') as total_orders,
        (SELECT COUNT(*) FROM sales WHERE rider_id = u.user_id) as total_deliveries
        FROM users u
        WHERE u.role = ?
        ORDER BY u.created_at DESC";
$users = $db->fetchAll($sql, [$user_type]);
